...
1. **Read both protocol files**
   - Read `study-areas/base.md` for shared execution rules.
   - Read the selected `study-areas/{NN}-{name}.md` for the study content.

2. **Analyze each reference repo**
   - For every repo in the repo list, inspect the source following the selected protocol.
   - Prefer implementation, tests, configuration, and public interfaces over README-level claims.
   - Write findings to `reports/repo/{NN}-{protocol-name}/{repo-name}.md` using `templates/repo-analysis.md`.

3. **Analyze HelloSales**
   - Analyze `HelloSales/` against the same protocol and evidence standard.
   - Write findings to `reports/repo/{NN}-{protocol-name}/hellosales.md` using `templates/repo-analysis.md`.

4. **Create the combined report** (after all repos are analyzed)
   - After all per-repo analyses exist, read each one before synthesizing.
   - Write the report to `reports/final/{NN}-{protocol-name}.md` using `templates/report.md`.
...
```
reports/repo/{NN}-{protocol-name}/
├── {repo-1}.md
├── {repo-2}.md
├── {repo-3}.md
└── hellosales.md

reports/final/{NN}-{protocol-name}.md
```
